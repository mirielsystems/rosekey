	...
	protected async tickMinor(): Promise<Partial<KVs<typeof schema>>> {
		...
		const [sub, pub, pubsub, subActive, pubActive] = await Promise.all([
			this.followingsRepository.createQueryBuilder('following')
				...
				.andWhere(meta.enableAllowedHostsInWhiteList ? meta.allowedHosts.length === 0 ? '1=0' : 'following.followeeHost ILIKE ANY(ARRAY[:...blocked])' : '1=1', { blocked: meta.allowedHosts.flatMap(x => [x, `%.${x}`]) })
			...
			this.followingsRepository.createQueryBuilder('following')
				...
				.andWhere(meta.enableAllowedHostsInWhiteList ? meta.allowedHosts.length === 0 ? '1=0' : 'following.followerHost ILIKE ANY(ARRAY[:...blocked])' : '1=1', { blocked: meta.allowedHosts.flatMap(x => [x, `%.${x}`]) })
			...
			this.instancesRepository.createQueryBuilder('instance')
				...
				.andWhere(meta.enableAllowedHostsInWhiteList ? meta.allowedHosts.length === 0 ? '1=0' : 'instance.host ILIKE ANY(ARRAY[:...blocked])' : '1=1', { blocked: meta.allowedHosts.flatMap(x => [x, `%.${x}`]) })
