...
const $userGroupsRepository: Provider = {
	...
	useFactory: (db: DataSource) => db.getRepository(MiUserGroup),
	inject: [DI.db],
};

const $userGroupJoiningsRepository: Provider = {
	provide: DI.userGroupJoiningsRepository,
	useFactory: (db: DataSource) => db.getRepository(MiUserGroupJoining),
	inject: [DI.db],
};

const $userGroupInvitationsRepository: Provider = {
	provide: DI.userGroupInvitationsRepository,
	useFactory: (db: DataSource) => db.getRepository(MiUserGroupInvitation),
	inject: [DI.db],
};
...
const $messagingMessagesRepository: Provider = {
	...
	useFactory: (db: DataSource) => db.getRepository(MiMessagingMessage),
	inject: [DI.db],
};
...
const $subscriptionPlansRepository: Provider = {
	...
	useFactory: (db: DataSource) => db.getRepository(SubscriptionPlan),
	inject: [DI.db],
};
