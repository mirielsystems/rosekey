				...
				<div class="_gaps_m">
					...
					<template
						v-if="
							...
							version.length > 0 &&
							releasesCherryPick &&
							releasesCherryPick.length > 0
						"
					>
						<FormInfo
							v-if="
								compareVersions(version, releasesCherryPick[0]?.tag_name) > 0
							"
						...
						>
						<FormInfo v-else warn>{{
							i18n.ts.newVersionOfClientAvailable
						}}</FormInfo>
					</template>
					...
					<FormSection first>
						...
						<MkKeyValue @click="whatIsNewCherryPick">
							<template #key
								>{{ i18n.ts.currentVersion }} <i class="ti ti-external-link"></i
							></template>
						...
						<MkKeyValue
							v-if="
								version &&
								releasesCherryPick &&
								version < releasesCherryPick[0]?.tag_name &&
								!skipVersion
							"
						...
						>
							<template #key
								>{{ i18n.ts.latestVersion }} <i class="ti ti-external-link"></i
							></template>
							<template v-if="releasesCherryPick" #value>{{
						...
						<MkButton
							...
							>{{ i18n.ts.skipThisVersion }}</MkButton
						>
					</FormSection>

					<FormSection @click="whatIsNewLatestCherryPick">
						<template #label
							>Rosekey <i class="ti ti-external-link"></i
						></template>
						<MkKeyValue>
						...
						>
							<template v-if="releasesCherryPick" #value
								><MkTime
									:time="releasesCherryPick[0]?.published_at"
									mode="detail"
							/></template>
							<template v-else #value><MkEllipsis /></template>
						</MkKeyValue>
					...
					<FormSection @click="whatIsNewLatestMisskey">
						<template #label
							>Misskey <i class="ti ti-external-link"></i
						></template>
						<MkKeyValue>
							<template #key>{{ i18n.ts.latestVersion }}</template>
							<template v-if="releasesMisskey" #value>{{
								releasesMisskey[0]?.tag_name
							}}</template>
							<template v-else #value><MkEllipsis /></template>
						</MkKeyValue>
						<MkKeyValue
						...
						>
							<template v-if="releasesMisskey" #value
								><MkTime :time="releasesMisskey[0]?.published_at" mode="detail"
							/></template>
							<template v-else #value><MkEllipsis /></template>
						</MkKeyValue>
					</FormSection>
...
async function init() {
	...
	try {
		...
		const cherryPickResponse = await fetch(
			"https://api.github.com/repos/kokonect-link/cherrypick/releases",
		);
		const cherryPickData = await cherryPickResponse.json();
		releasesCherryPick.value = meta.enableReceivePrerelease
			? cherryPickData
			: cherryPickData.filter((x) => !x.prerelease);

		if (
			compareVersions(
				skipCherryPickVersion.value,
				releasesCherryPick.value[0].tag_name,
			) < 0
		) {
	...
	try {
		...
		const misskeyResponse = await fetch(
			"https://api.github.com/repos/misskey-dev/misskey/releases",
		);
		const misskeyData = await misskeyResponse.json();
		releasesMisskey.value = meta.enableReceivePrerelease
			? misskeyData
			: misskeyData.filter((x) => !x.prerelease);
	} catch (error) {
...
const whatIsNewCherryPick = () => {
	window.open(
		`https://code.rosekey.dev/miriel/rosekey/blob/master/CHANGELOG.md#${version.replace(/\./g, "")}`,
		"_blank",
	);
};
...
const whatIsNewLatestCherryPick = () => {
	window.open(
		`https://code.rosekey.dev/miriel/rosekey/blob/release/CHANGELOG.md#${releasesCherryPick.value[0]?.tag_name.replace(/\./g, "")}`,
		"_blank",
	);
};
...
const whatIsNewMisskey = () => {
	window.open(
		`https://misskey-hub.net/docs/releases.html#_${basedMisskeyVersion.replace(/\./g, "-")}`,
		"_blank",
	);
};
...
const whatIsNewLatestMisskey = () => {
	window.open(
		`https://github.com/misskey-dev/misskey/blob/develop/CHANGELOG.md#${releasesMisskey.value[0]?.tag_name.replace(/\./g, "")}`,
		"_blank",
	);
};
