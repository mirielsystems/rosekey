...
<template>
		...
		<FormSuspense :p="init">
		  <div class="_gaps_m">
			...
			<FormSection first>
			  ...
			  <MkKeyValue v-if="version && releasesCherryPick && version < releasesCherryPick[0]?.tag_name && !skipVersion" style="margin-top: 10px;" @click="whatIsNewLatestCherryPick">
			  ...
			  <MkButton v-if="!skipVersion && releasesCherryPick && compareVersions(version, releasesCherryPick[0]?.tag_name) < 0" style="margin-top: 10px;" @click="skipThisVersion">{{ i18n.ts.skipThisVersion }}</MkButton>
			</FormSection>
  ...
  const whatIsNewLatestCherryPick = () => {
	window.open(`https://code.rosekey.dev/miriel/rosekey/blob/release/CHANGELOG.md#${releasesCherryPick.value[0]?.tag_name.replace(/\./g, '')}`, '_blank');
  };
