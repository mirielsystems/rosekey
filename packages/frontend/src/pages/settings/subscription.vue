			...
			<div class="_gaps">
				<div v-for="plan in items" :key="plan.id" class="_panel" :class="$style.plan">
					<div :class="$style.planBody">
						...
						<div>
							<MkButton v-if="currentPlan === null" primary :class="$style.button" @click="subscribe(plan)"><i class="ti ti-plus"></i>{{ i18n.ts._subscription.subscribe }}</MkButton>
							...
							<MkButton v-else :class="$style.button" @click="change(plan)"><i class="ti ti-reload"></i>{{ i18n.ts._subscription.changePlan }}</MkButton>
						</div>
					</div>
				</div>
			</div>
